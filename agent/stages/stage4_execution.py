    ...
    def execute_python_tests(self, repo_path: str, generated_tests: List[Dict[str, Any]], state: AgentState) -> Tuple[bool, Optional[Dict[str, Any]]]:
        ...
        for attempt in range(max_dependency_retries):
            try:
                ...
                # Check if there is a ModuleNotFoundError or ImportError
                combined_output = (result.stdout or "") + "\n" + (result.stderr or "")
                missing_module_match = re.search(r"(ModuleNotFoundError|ImportError):\s+No module named\s+'([^']+)'", combined_output)
                
                if missing_module_match:
                    missing_module = missing_module_match.group(2)
                    if "." in missing_module:
                        package_to_install = missing_module.split(".")[0]
                    else:
                        package_to_install = missing_module
                    
                    # Avoid trying to pip install project internal packages
                    is_internal = False
                    for py_p in python_paths:
                        if os.path.exists(os.path.join(py_p, package_to_install)):
                            is_internal = True
                            break
                    
                    if is_internal:
                        logger.warning(f"Missing internal module '{missing_module}'. This is a code mapping/import path error. Breaking loop.")
                        break
                    
                    logger.info(f"Auto-healing: Detected missing third-party dependency '{package_to_install}'. Running 'pip install {package_to_install}'...")
                    try:
                        pip_res = subprocess.run(
                            ["pip", "install", "--no-cache-dir", package_to_install],
                            stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE,
                            text=True,
                            timeout=60
                        ...
                        if pip_res.returncode == 0:
                            logger.info(f"Successfully auto-installed '{package_to_install}'. Retrying test execution...")
                            continue
                        else:
                            logger.error(f"Failed to auto-install '{package_to_install}': {pip_res.stderr}")
                            break
                    except Exception as pip_err:
                        logger.error(f"Error executing pip install for {package_to_install}: {pip_err}")
                        break
